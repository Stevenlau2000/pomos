...
// 学科枚举（KG_BOARDS）扩为 10（5 经典物理板块 + 5 新学科：高数/矢量分析/线性代数/理论力学/电动力学）。
export const KG_BOARDS = [
  "力学",
  "电磁学",
  "热学",
  "光学",
    ...
    problems: [
      {
        ...
        topic: "泰勒展开·RC 暂态",
        difficulty: 3,
        stem: "RC 电路充电 q(t)=Cε(1−e^{−t/RC})，求 t≪RC 时电量的近似表达式（至二阶）。",
        hint: "e^{−x}≈1−x+x²/2，代入 x=t/RC。",
        solutionPoints: [
          "e^{−t/RC}≈1−t/RC+(t/RC)²/2",
          "q≈Cε·(t/RC−(t/RC)²/2)=Cεt/(RC)−Cεt²/(2R²C²)",
        ],
        keyPoints: ["泰勒展开", "小量近似保留最低非零阶"],
      },
    ],
  },
  矢量分析: {
    ...
    problems: [
      {
        ...
        hint: "r≠0 时 ∇·(r̂/r²)=0；仅在原点有源，用高斯定理求总通量。",
        solutionPoints: [
          "r≠0 时 ∇·(r̂/r²)=0",
          "取包围原点的小球，通量=q/ε₀ ⟹ 散度为 δ 源 q/ε₀δ³(r)",
        ],
        keyPoints: ["散度是标量源密度", "δ 函数描述点源"],
      },
    ],
  },
  线性代数: {
    ...
    problems: [
      {
        ...
        topic: "特征值·简正模",
        difficulty: 4,
        stem: "双耦合振子质量 m、弹簧 k，位移 x₁,x₂，运动矩阵方程 M ẍ+Kx=0，其中 M=mI、K=k[[2,−1],[−1,2]]。求两简正模频率。",
        hint: "求 K 的特征值 λ，频率 ω=√(λ/k_eff)，此处有效刚度按 λ。",
        solutionPoints: [
          "K 特征值：λ₁=k，λ₂=3k（对应 [1,1]、[1,−1]）",
          "ω₁=√(k/m)，ω₂=√(3k/m)",
        ],
        keyPoints: ["特征向量即简正坐标", "频率由特征值开方"],
      },
    ],
  },
  理论力学: {
    ...
    problems: [
      {
        ...
        topic: "拉格朗日·单摆",
        difficulty: 4,
        stem: "质量 m 长 l 单摆，以 θ 为广义坐标写出 L 并导出小角运动方程。",
        hint: "T=½ml²θ̇²，V=mgl(1−cosθ)。",
        solutionPoints: [
          "L=½ml²θ̇²−mgl(1−cosθ)",
          "拉氏方程：ml²θ̈+mgl sinθ=0",
          "小角 sinθ≈θ ⟹ θ̈+(g/l)θ=0",
        ],
        keyPoints: ["L=T−V", "广义坐标降阶"],
      },
    ],
  },
  电动力学: {
    ...
    problems: [
      {
        ...
        hint: "由法拉第定律 ∇×E=−∂B/∂t 推出 B，并验证 k=ω/c。",
        solutionPoints: [
          "由 ∇×E=−∂B/∂t 得 B=(E₀/c)cos(kx−ωt)ẑ",
          "且 k=ω/c、c=1/√(μ₀ε₀)",
        ],
        keyPoints: ["c=1/√(μ₀ε₀)", "E、B、传播方向两两正交"],
      },
    ],
  },
};
...
export function findBoardByKeyword(text: string): Board | null {
  ...
  const rules: [Board, string[]][] = [
    ...
    ["高数", ["高数", "高等数学", "泰勒", "级数", "微分方程", "微分", "极限", "近似"]],
    ["矢量分析", ["矢量", "向量", "梯度", "散度", "旋度", "场论", "nabla", "高斯定理"]],
    ["线性代数", ["线性代数", "矩阵", "行列式", "特征值", "特征向量", "简正模"]],
    ["理论力学", ["理论力学", "拉格朗日", "哈密顿", "分析力学", "广义坐标", "正则方程"]],
    ["电动力学", ["电动力学", "麦克斯韦", "电磁波", "位移电流", "能流"]],
  ];
